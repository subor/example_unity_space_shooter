...
{
    public delegate void LoadCallbackType(string fileName);

    public int defaultWidth = 100;
    public int minWidth = 50;
    public GUISkin skin;

    LoadCallbackType LoadCallback;
    bool WindowOpen = false;
    WindowType WinType;
    String MainFolder = "";
    String FileToSave = "";
    String NameOfFileToSave = "";
    PathInfo[] PathTree = new PathInfo[0];
    ...
    public void OpenFile(String mainFolder, LoadCallbackType callback)
    {
    ...
    public void OpenFile(String mainFolder, String[] extensions, LoadCallbackType callback)
    {
        LoadCallback = callback;
        if (Directory.Exists(mainFolder))
        {
            ...
            MainFolder = mainFolder; // example: Application.dataPath/Saves;
            GetAllSubDirectoriesAndFiles(MainFolder, extensions);
    ...
    void DrawBrowser(int windowID)
    {
        ...
        if (WinType == WindowType.Open && GUI.Button(bottom, "Open"))
        {
            ...
            if (File.Exists(FileToOpen) || Directory.Exists(FileToOpen))
            {
                ...
                if (LoadCallback != null)
                {
                    LoadCallback(FileToOpen);
                }
            }
            else
            {
                Debug.LogError("The File you are trying to open must have gotten moved, or deleted.");
                if (LoadCallback != null)
                {
                    LoadCallback(null);
                }
    ...
    {
        PathTree = new PathInfo[0];
        WalkTheTree(new DirectoryInfo(MainDirectory));
    }

    void GetAllSubDirectoriesAndFiles(String MainDirectory, String[] extensions)
    {
        PathTree = new PathInfo[0];
        WalkTheTree(new DirectoryInfo(MainDirectory), extensions);
    }
    ...
    void WalkTheTree(DirectoryInfo DI)
    {
        ...
        //Debug
        PathInfo[] PathTreeTemp = new PathInfo[diArr.Length + fiArr.Length];
    ...
    void WalkTheTree(DirectoryInfo DI, String[] extensions)
    {
        String PARENT = DI.FullName;
        DirectoryInfo[] diArr = DI.GetDirectories();
        FileInfo[] fiArr = DI.GetFiles();
        ...
        foreach (var x in fiArr)
        {
            var components = x.Name.Split('.');
            if (Array.IndexOf(extensions, components[components.Length - 1]) >= 0)
            {
                allowedFiles.Add(x);
            }
        }

        fiArr = allowedFiles.ToArray();
    ...
    PathInfo[] AddPathArrays(PathInfo[] IN1, PathInfo[] IN2)
    {
        PathInfo[] Temp = new PathInfo[IN1.Length + IN2.Length];
        IN1.CopyTo(Temp, 0);
